"""
DocuSense - QA Endpoint Integration Tests
...
def mock_session_service():
    ...
    with patch("app.api.v1.endpoints.qa.SessionService") as mock:
...
def mock_qa_service():
    """Mock the QA service."""
    with patch("app.api.v1.endpoints.qa.QAService") as mock:
        mock_instance = MagicMock()
        mock.return_value = mock_instance
        yield mock_instance
...
def mock_model_loader():
    ...
    with patch("app.api.v1.endpoints.qa.get_model_loader") as mock:
        mock_instance = MagicMock()
...
class TestQAEndpoint:
    """Tests for POST /api/v1/qa endpoint."""
    ...
    def test_ask_question_qa_unavailable(
        ...
        with patch("app.api.v1.endpoints.qa.get_model_loader") as mock_loader:
            mock_instance = MagicMock()
...
class TestQACacheEndpoint:
    """Tests for DELETE /api/v1/qa/cache endpoint."""

    def test_clear_cache(self, client):
        """Test clearing the QA cache."""
        with patch("app.api.v1.endpoints.qa.QAService") as mock_service:
            mock_service.clear_cache.return_value = 5
            
            response = client.delete("/api/v1/qa/cache")
...
class TestQAStatusEndpoint:
    """Tests for GET /api/v1/qa/status endpoint."""
...
class TestRateLimiting:
    """Tests for rate limiting on QA endpoint."""
    ...
    def test_rate_limit_exceeded(self, client, mock_session_service, mock_model_loader):
        ...
        with patch("app.api.v1.endpoints.qa.limiter") as mock_limiter:
